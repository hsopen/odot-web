...
function handleLogin() {
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      try {
        const result = await axios.post(`${import.meta.env.VITE_API_HOST}/api/v1/auth/loginUser`, form.value, { withCredentials: true })
            ...
            <NCard class="login-card">
              ...
              <NForm ref="formRef" @submit.prevent="handleLogin" @keydown.enter="handleLogin">
                ...
                <NSpace justify="space-between" class="login-options">
                  <NCheckbox v-model:checked="form.rememberMe">
                    {{ $t('rememberToLogIn') }}
                  </NCheckbox>
                  <NA href="#">
                    {{ $t('forgetThePassword') }}
